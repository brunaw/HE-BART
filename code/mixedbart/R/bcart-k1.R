...
bcart_k1 <- function(formula, dataset, iter = 5000, group_variable, pars,
                     scale = TRUE, p_grow = 0.5){
  ...
  for(i in 1:iter){
    ...
    new_k1 <- round(runif(n = 1, min = 5, max = 12), 2)
    # r_k[i] <- lk_ratio_k(new_k1, pars$k1,
