...
bcart_fixed_k1 <- function(formula, dataset, iter = 5000, 
                           group_variable, pars,
                           scale = FALSE, p_grow = 0.5, 
                           min_u = 5, max_u = 12, prior_k1 = FALSE, 
                           stump = FALSE){
  ...
  # Handling data
  #---------------------------------------------------------------------
  results_ds <- data_handler(formula, dataset, group_variable,
                             scale_fc = FALSE)
  data <- results_ds$data
  N <- n <- nrow(data)
  group <- results_ds$group
  y <- data$y
  X <- results_ds$X
  #---------------------------------------------------------------------
  ...
  M <- n_distinct(group)
  M_m <- model.matrix(~ factor(group) - 1)
  beta <- pars$beta
  alpha <- pars$alpha
  mu_mu <- pars$mu_mu
  k1 <- pars$k1
  ...
  for(i in 1:iter){
    ...
    for(m in 1:n_nodes){
      ...
      for(j in sort(as.numeric(unique(group)))){
