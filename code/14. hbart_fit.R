...
#   mutate(current_tree = map(tree_data, ~{ slice(.x, (25))})) %>% 
#   select(tree_index, current_tree) %>% 
#   unnest(current_tree) %>% 
#   select(tree_index, est_tree) %>% 
#   unnest(est_tree) %>% 
...
# p22
# 
# #ggsave(file = "results/reports/january/tau.png")
# 
...
# p33
# 
# #ggsave(file = "results/reports/january/k1.png")
# 
#----------------------------------------------------------------------
# rr <- res %>% 
#   select(pred, y, group) %>% 
...
#ggsave(file = "results/reports/january/residuals_test.png")
#-------------------------------------------------------------------------
#-------------------------------------------------------------------------
df_real <- sleepstudy %>% 
...
#ggsave(file = "results/reports/january/sleepstudy.png")
...
ggsave(file = "results/reports/january/residuals_sleepstudy_test.png")
...
ggsave(file = "results/reports/january/residuals_sleepstudy_train.png")
#-------------------------------------------------------------------------
#-------------------------------------------------------------------------
library(brolgar)
data(wages)
...
# trees are growing in the same rule and same variable; stop that
# predict seem to be working
# res <- m0_real$final_trees$results %>% 
#   map(drop_na)
# res
...
lm3_m0 <- lmer(y ~ xp + unemploy_rate + (1 |id), data = train)
pred_lm3_m0 <- predict(lm3_m0, test)
pred_lm3_m0_train <- predict(lm3_m0)
