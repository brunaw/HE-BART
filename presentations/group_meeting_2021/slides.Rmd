...
- $P(\mathcal{T})$: does not have a close form, but can defined as 
...
# BCART
  ...
  at a rule $\rho$ in terminal node $\eta$; 
    - depends on the number of available predictors and their range of available values of the node of $\eta$
...
structure  $\mathcal{T}$, and terminal node parameters $M$
...
generates posterior draws from $P(\mathcal{T_1^{M}},\dots,\mathcal{T_m^{M}}, \tau | \mathbf{y})$.
...
where $\Psi =  k_1 MM^{T} + \mathbb{I}$, $\bar R_j n_j$ = residual mean, 
$n_j$ = # obs per group 
